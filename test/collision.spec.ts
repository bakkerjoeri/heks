...
describe('isPointInRectangle', () => {
	test('returns true when the point is within the rectangle', () => {
		...
		expect(isPointInRectangle([2, 3], [[1, 1], [3, 3]])).toBe(true);
	});
...
describe('isRectangleInRectangle', () => {
	test('returns true when the rectangles overlap', () => {
		expect(isRectangleInRectangle([[0, 0], [10, 10]], [[0, 0], [10, 10]])).toBe(true);
	...
	test('returns false when edges touch but do not overlap', () => {
		...
		expect(isRectangleInRectangle([[1, 1], [3, 3]], [[1, 0], [3, 1]])).toBe(false);

		// Right edge of a is touching left edge of b
		expect(isRectangleInRectangle([[1, 1], [3, 3]], [[3, 1], [4, 3]])).toBe(false);

		// Bottom edge of a is toudhing top edge of b
		expect(isRectangleInRectangle([[1, 1], [3, 3]], [[1, 3], [3, 4]])).toBe(false);

		// Bottom edge of a is toudhing top edge of b
		expect(isRectangleInRectangle([[1, 1], [3, 3]], [[0, 1], [1, 3]])).toBe(false);
	});
})
